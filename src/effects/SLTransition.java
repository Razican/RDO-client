 ...
 * not be very nice. Instead, you can set its starting side, the side from where
 * it should be brought into view to its target place. It will then slide from
 * this side to its target place.
 * <p/>
...
public class SLTransition {
	 ...
	 * If another transition was already running, nothing happens and the method
	 * returns false;
	 */
	public boolean play()
	{
	...
	private void tween(final SLKeyframe kf)
	{
		...
		for (Component c: kf.getCmps())
		{
			...
			{
				timeline.push(Tween
				.to(c, SLAnimator.ComponentAccessor.XYWH, duration)
				.target(t.x, t.y, t.w, t.h).delay(kf.getDelay(c)));
			}
			else if (animXY)
			{
				timeline.push(Tween
				.to(c, SLAnimator.ComponentAccessor.XY, duration)
				.target(t.x, t.y).delay(kf.getDelay(c)));
			}
			else if (animWH)
			{
				timeline.push(Tween
				.to(c, SLAnimator.ComponentAccessor.WH, duration)
				.target(t.w, t.h).delay(kf.getDelay(c)));
			}
		}
		...
		timeline.setCallback(new TweenCallback()
		{
			...
			public void onEvent(int type, BaseTween<?> source)
			{
				...
				{
					currentKeyframe++;
					play(keyframes.get(currentKeyframe),
